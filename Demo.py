...
left_disparity, right_disparity = disparity_estimator.get_sparse_disparity()

cv2.imwrite("disparityL.png", left_disparity)
cv2.imwrite("disparityR.png", right_disparity)

cv2.imshow("image left", left_image)
cv2.imshow("Disparity left", left_disparity/16)
cv2.imshow("Disparity right", right_disparity/16)
cv2.waitKey(0)
